...
function httpGet(cmd) {
  var url = "http://192.168.20.1/services/pi_proxy.php"
...
Turtle.step = function(command, values) {
  switch (command) {
    case 'FD':  // Forward
      ...
      if (distance) {
        Turtle.x += (distance * 50) * Math.sin(2 * Math.PI * Turtle.heading / 360);
        Turtle.y -= (distance * 50) * Math.cos(2 * Math.PI * Turtle.heading / 360);
        var bump = 0;
      } else {
      ...
      if (distance > 0) {
        executa_instrucao('pf ' + distance);
      }
      else {
        executa_instrucao('pt ' + distance * -1); 
      }
      break;
    case 'RT':  // Right Turn
